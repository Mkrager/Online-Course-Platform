...
namespace OnlineCoursePlatform.Application.UnitTests.Mocks
{
    public class RepositoryMocks
    {
        ...
        public static Mock<IAsyncRepository<OnlineCoursePlatform.Domain.Entities.Payment>> GetPaymantRepository()
        {
            ...
            {
                new OnlineCoursePlatform.Domain.Entities.Payment()
                {
                    Id = Guid.Parse("4f50d45e-f395-4688-a55f-c64e06649572")
                }
            };
            
            var mockRepository = new Mock<IAsyncRepository<OnlineCoursePlatform.Domain.Entities.Payment>>();

            mockRepository.Setup(repo => repo.ListAllAsync())
                .ReturnsAsync(payments);

            mockRepository.Setup(repo => repo.AddAsync(It.IsAny<OnlineCoursePlatform.Domain.Entities.Payment>()))
                .ReturnsAsync((OnlineCoursePlatform.Domain.Entities.Payment payment) =>
                {
                    payments.Add(payment);
                    return payment;
                });

            mockRepository.Setup(r => r.UpdateAsync(It.IsAny<OnlineCoursePlatform.Domain.Entities.Payment>()))
                .Callback((OnlineCoursePlatform.Domain.Entities.Payment course) =>
                {
